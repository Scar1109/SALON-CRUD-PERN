...
function AdminTestimonials() {
    ...
    useEffect(() => {
        const tempList = testimonials.filter(
            (testimonial) =>
                (testimonial.id && testimonial.id.toString().toLowerCase().includes(searchTerm.toLowerCase())) ||
                ...
                (testimonial.status && testimonial.status.toLowerCase().includes(searchTerm.toLowerCase())) ||
                (testimonial.rating && testimonial.rating.toString().includes(searchTerm))
        );
        ...
        <div className="flex h-screen">
            ...
            <div className="w-[80%] h-full bg-pink-500 p-4 julius-sans-one-regular">
                <div className="p-6 bg-white rounded-lg shadow-md overflow-x-auto">
                    ...
                    <div className="mb-6">
                        <h2 className="text-xl font-semibold mb-4">Customer Rating Table</h2>
                                ...
                                {filteredTestimonials.length > 0 ? (
                                    filteredTestimonials.map((testimonial) => (
                                        <tr key={testimonial.id} className="border-b">
                                            <td className="py-2 px-4 text-gray-700">{testimonial.id}</td>
                                            <td className="py-2 px-4 text-gray-700">{testimonial.rating}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
                            ...
                            <tbody>
                                {filteredTestimonials.map((testimonial) => (
                                    <tr key={testimonial.id} className="border-b border-gray-300">
                                        <td className="py-2 px-4 text-gray-700">{testimonial.id}</td>
                                        <td className="py-2 px-4 text-gray-700">{testimonial.customerEmail || 'N/A'}</td>
                                        <td className="py-2 px-4 text-gray-700">{testimonial.title}</td>
                                        <td className="py-2 px-4 text-gray-700">{testimonial.description}</td>                                        
                                        <td className="py-2 px-4 text-gray-700">{testimonial.status}</td>
